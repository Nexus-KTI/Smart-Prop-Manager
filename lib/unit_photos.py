"""One public unit photo for the apply page (`unit-photos` bucket)."""
...
def validate_unit_photo(content_type: str, file_bytes: bytes) -> str:
    """Return the file extension, or raise ValueError before any storage call."""
    if not file_bytes:
...
def upload_unit_photo(
    *,
    ...
    """Upload one unit photo; return the public URL."""
    from lib.db import create_service_client

    ext = validate_unit_photo(content_type, file_bytes)
    client = create_service_client()
    _ensure_photos_bucket(client)

    path = f"{user_id}/{unit_id}/{uuid.uuid4().hex}.{ext}"
